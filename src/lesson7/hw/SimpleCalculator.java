package lesson7.hw;

public class SimpleCalculator implements Calculator {

    @Override
    public int calculateTotal(CoinBundle enteredCoins) {

        return enteredCoins.getTotal();
    }

    @Override
    public CoinBundle calculateChange(int amountMoneyToReturn) {
        CoinBundle change = new CoinBundle(new int[4]);
        int remainingAmount = amountMoneyToReturn;

        change.number10RublesCoins = remainingAmount / 10;
        remainingAmount = remainingAmount % 10;

        change.number5RublesCoins = remainingAmount / 5;
        remainingAmount = remainingAmount % 5;

        change.number2RublesCoins = remainingAmount / 2;
        remainingAmount = remainingAmount % 2;

        change.number1RublesCoins = remainingAmount / 1;
        remainingAmount = remainingAmount % 1;

        return change;
    }
}
